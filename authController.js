...
class authController {
    ...
    async updateUser(req, res) {
        try {
            const {id, password, email, first_name, last_name, age, address} = req.body
            const hashPassword = await bcrypt.hashSync(password, 7)
            await db.query('UPDATE person SET password = $1, email = $2, first_name = $3, last_name = $4, age = $5, address = $6 WHERE id = $7',
                [hashPassword, email, first_name, last_name, age, address, id],
                (err, result) => {
                    if (err) {
                        console.log(err.stack)
                        res.status(400).json({message: 'Update user error, BD error'})
                    } else {
                        res.json(result.rows[0]) // need delete, debug
    ...
    async deleteUser(req, res) {
        try {
            const errors = validationResult(req)
            if (!errors.isEmpty()){
                return res.status(400).json({message: 'Delete user error ', errors})
            }
            const {email, password} = req.body
            const user = await db.query('SELECT * FROM person WHERE email = $1', [email])
            if (user.rows.length === 0) {
            ...
            await db.query('DELETE FROM person WHERE email = $1', [email])
